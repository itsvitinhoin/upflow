...
const levelClass: Record<PermissionLevel, string> = {
  ...
  none: "border-white/10 bg-white/[0.03] text-muted-foreground",
  ...
  return (
    <>
      ...
      <main className="mx-auto w-full max-w-6xl space-y-5 overflow-x-hidden p-4 sm:p-6">
        <section className="rounded-3xl border border-blue-300/15 bg-[linear-gradient(135deg,rgba(37,99,235,0.18),rgba(15,23,42,0.94))] p-6 shadow-[0_0_40px_rgba(37,99,235,0.12)]">
          <Link
            href="/settings"
        ...
        <section className="grid gap-3 md:grid-cols-4">
          {permissionRoles.map((role) => (
            <article key={role.id} className="rounded-2xl border border-white/10 bg-card/70 p-4">
        ...
        <section className="space-y-4">
          {permissionMatrixSections.map((section) => (
            <article key={section.title} className="overflow-hidden rounded-2xl border border-white/10 bg-card/70">
                ...
                <table className="w-full min-w-[760px] text-left text-sm">
                  ...
                  <tbody>
                    {section.capabilities.map((capability) => (
                      <tr key={capability.label} className="border-b border-white/5 last:border-0">
