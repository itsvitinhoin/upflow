...
export default function OperationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || `Could not load operations (${res.status})`);
      setPayload(data);
    } catch (err) {
        ...
        <section className="rounded-2xl border border-blue-400/20 bg-[radial-gradient(circle_at_top_left,rgba(37,99,235,0.2),transparent_34%),rgba(2,6,23,0.84)] p-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-4 text-2xl font-bold text-white">Operational readiness center</h1>
              <p className="mt-2 max-w-3xl text-sm text-muted-foreground">
                One view for failed automations, report approvals, client risk, permission changes, invite failures, and health verification.
              </p>
            </div>
        ...
        {loading || !payload ? (
          ...
          <>
            <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
              <OpsCard label="Failed automations" value={payload.counts.failed_automations} icon={<Bot className="h-4 w-4" />} danger />
              <OpsCard label="Approvals waiting" value={payload.counts.approvals_waiting} icon={<CheckCircle2 className="h-4 w-4" />} />
              <OpsCard label="Reports waiting" value={payload.counts.report_approvals_waiting} icon={<FileText className="h-4 w-4" />} />
              <OpsCard label="Clients at risk" value={payload.counts.clients_at_risk} icon={<HeartPulse className="h-4 w-4" />} danger />
              <OpsCard label="Invite failures" value={payload.counts.invite_failures} icon={<UserPlus className="h-4 w-4" />} danger />
              <OpsCard label="Permission changes" value={payload.counts.permission_changes_7d} icon={<ShieldCheck className="h-4 w-4" />} />
            </section>

            <section className="grid gap-4 xl:grid-cols-[1fr_380px]">
              <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
                <h2 className="text-sm font-semibold text-white">Latest automation runs</h2>
                <div className="mt-3 divide-y divide-white/10">
                  {payload.latest_runs.length === 0 ? (
                    <p className="py-6 text-sm text-muted-foreground">No automation runs yet.</p>
                  ) : (
                    payload.latest_runs.map((run) => (
                      <div key={run.id} className="grid gap-2 py-3 text-sm sm:grid-cols-[1fr_auto]">
                        ...
                        <p className="text-xs text-muted-foreground">{run.executed} executed / {run.skipped} skipped</p>
                      </div>
                    ))
                  )}
                </div>
              ...
              <aside className="space-y-4">
                <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
                  <h2 className="text-sm font-semibold text-white">Clients needing attention</h2>
                  <div className="mt-3 space-y-2">
                    {payload.clients_at_risk.length === 0 ? (
                      <p className="rounded-lg border border-white/10 bg-white/[0.03] px-3 py-3 text-xs text-muted-foreground">No current client risk records.</p>
                    ) : (
                ...
                <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
                  <h2 className="text-sm font-semibold text-white">Hard verification</h2>
                  <div className="mt-3 grid gap-2">
                    <Link href="/api/health" className="text-sm text-blue-100 hover:text-white">Open /api/health</Link>
                    <Link href="/admin/health" className="text-sm text-blue-100 hover:text-white">Open /admin/health</Link>
                  </div>
                </div>
              </aside>
            </section>
